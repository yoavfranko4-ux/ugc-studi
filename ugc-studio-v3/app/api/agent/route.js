...
Return ONLY JSON:
{
  ...
  // ── STEP 2: Nano Banana — תמונות ישירות, ללא תיאורים ──
  const { productImageUrl } = req_data
  const frames = []
  for (let i = 0; i < 4; i++) {
    try {
      // שלח את תמונות הרפרנס ישירות
      const imageUrls = [avatarUrl] // אווטאר תמיד ראשון
      if (i > 0 && frames[i-1]) imageUrls.push(frames[i-1]) // frame קודם להמשכיות
      if (productImageUrl && i >= 1) imageUrls.push(productImageUrl) // מוצר לסצנות 2-4
