...
Return ONLY JSON:
{
  ...
  // ── STEP 2: Generate Nano Banana frames SEQUENTIALLY ──
  // Send avatar + product images together for identity + product consistency
  const { productImageUrl } = req_data
  const frames = []
  for (let i = 0; i < 4; i++) {
    try {
      // Use avatar for scene 1, previous frame for scenes 2-4 (continuity)
      const refUrl = i === 0 ? avatarUrl : (frames[i - 1] || avatarUrl)
      
      // Build image_urls: always include original avatar + product if available
      const imageUrls = [refUrl]
      if (i > 0) imageUrls.unshift(avatarUrl) // keep original avatar as anchor
      if (productImageUrl) imageUrls.push(productImageUrl) // add product image
      
      // Strengthen prompt to force same person from reference
      const strengthenedPrompt = `Using the exact same person from the first reference image, identical face hair skin and appearance. ${story.scenes[i].nb_prompt}${productImageUrl && i >= 1 ? '. The product shown in the last reference image is clearly visible in hands.' : ''}`
      ...
      if (nbRes.ok) {
        ...
        frames.push(d.images?.[0]?.url || d.images?.[0] || refUrl)
      } else {
        frames.push(i === 0 ? avatarUrl : frames[i - 1])
      }
    } catch {
  ...
  if (elevenKey) {
    try {
      const vRes = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId || '73z5yvUD5zgBgz92lJMW'}`, {
        method: 'POST',
