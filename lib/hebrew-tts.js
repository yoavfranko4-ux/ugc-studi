...
// Words that commonly open a new sentence/clause. If we see one of these
// following a bare word with no punctuation between them, we insert a period.
...
// Feminine wrong-form auto-corrections — identical intent to the server-side
// map in route.js, so even if a voiceover slips through the validator the TTS
// text still gets cleaned up before hitting ElevenLabs.
const FEMININE_WRONG_FORMS = [
  [/הייתי מביכה/g, 'הייתי מובכת'],
  [/הייתי מבולבל(?!\S)/g, 'הייתי מבולבלת'],
  [/הייתי נבוך(?!\S)/g, 'הייתי נבוכה'],
  [/מביכה לחייך/g, 'מובכת לחייך'],
  [/מביכה לי/g, 'מובכת לי'],
]

export function enforceNaturalPauses(text) {
  if (!text || typeof text !== 'string') return text
  let out = text

  // 1) Insert a period before a sentence-starter that follows another word
  //    with only whitespace between them (no punctuation).
  //    "...נורא עד שגיליתי" -> "...נורא. עד שגיליתי"
  for (const starter of SENTENCE_STARTERS) {
    const re = new RegExp(`([\\u0590-\\u05FF\\u05D0-\\u05EAa-zA-Z])\\s+(${starter})(?=\\s)`, 'g')
    out = out.replace(re, (_, pre, w) => `${pre}. ${w}`)
  }

  // 2) Add a comma inside any long stretch (≥8 words) that has no internal
  //    comma — break at the nearest space around the middle.
  out = out.replace(/([^.!?,]+)/g, (segment) => {
    const trimmed = segment.trim()
    if (!trimmed) return segment
    const words = trimmed.split(/\s+/)
    if (words.length < 9) return segment
    ...
    return `${leadingWs}${left}, ${right}${trailingWs}`
  })

  // 3) Ensure the text ends with a terminator.
  out = out.trim()
  if (out && !/[.!?]$/.test(out)) out += '.'

  // 4) Normalize any double punctuation we may have introduced.
  out = out
    .replace(/,\s*,/g, ',')
    .replace(/\.\s*\./g, '.')
    .replace(/\s+,/g, ',')
...
export function cleanHebrewText(text) {
  ...
  // Safety net: repair feminine wrong-form active participles that sometimes
  // slip past the server-side validator.
  for (const [re, rep] of FEMININE_WRONG_FORMS) out = out.replace(re, rep)
  ...
  // Enforce natural pauses — break run-on sentences into proper clauses before
  // layering in the standard pause-inserting rules.
  out = enforceNaturalPauses(out)
