...
export default function Home() {
  ...
  const runAgent = async () => {
    if (!avatarUrl) return alert('בחר דמות')
    ...
    try {
      ...
      const agentRes = await fetch('/api/agent', {
        ...
        body: JSON.stringify({
          ...
          avatarUrl: finalAvatarUrl, falKey, elevenKey,
          voiceId: '73z5yvUD5zgBgz92lJMW'
        })
      })

      if (!agentRes.ok) throw new Error('Agent failed')

      setAgentStatus({ script: 'done', frames: 'done', videos: 'done', voice: 'active' })
      addLog('✅ Agent הצליח!', 'ok')
